                        ...
                        <div class="col-4">
                            <div class="card row mb-5">
                                <div class="col d-flex justify-content-between align-items-center py-3">
                                    <img src="${pageContext.request.contextPath}/images/repUser.png" alt="" class="usrIcon">
                                    <div class="datos">
                                        <p>${empleado.nombre}</p>
                                        <p>${empleado.id_cargo}</p>
                                        <p>${empleado.id_depto}</p>
                                    </div>

                                    <div class="botones d-flex flex-column">
                                        <form action="${pageContext.request.contextPath}/ServletEmpleados" method="get">
                                            <p>${empleado.id}</p>
                                            <input type="hidden" name="ide" value="${empleado.id}">
                                            <input type="hidden" name="op" value="r">
                                            <button class="btn btn-primary btn-sm my-1 bot" type="submit">editar</button>
                                        </form>
                                        <form action="${pageContext.request.contextPath}/ServletEmpleados" method="get">
                                            <input type="hidden" name="ide" value="${empleado.id}">
                                            <input type="hidden" name="op" value="d">
                                            <button class="btn btn-danger btn-sm my-1 bot"  type="submit">eliminar</button>
                                        </form>
